...
const loadLiveVideo = (liveVideos, accessTokenDic) => new Promise((resolve, reject) => {
  getliveVideoIdByDB()
    .then((dbdata) => {
...
const deleteOldVideo = (oldLiveVideos) => {
  ...
  const conditionQuery = oldLiveVideos.reduce((str, element, index) => `${index == 0 ? `(${str}` : `${str},`}'${element.videoId}'`, '');
...
const InsertLiveData = (newLiveVideos) => {
  ...
  const rawQuery = newLiveVideos.reduce((str, {
    channelId, videoId, activeLiveChatId, startDate
  }) => `${str}('${channelId}', '${videoId}', '${activeLiveChatId}', '${startDate}'),`, '');
...
const InsertMetaData = (newLiveVideos) => {
  ...
  }) => `${str}( '${videoId}', '${videoTitle}', '${channelId}', '${channelName}', '${startDate}'),`, '');
  const conditionQuery = `${rawQuery.slice(0, -1)};`;
...
const getliveVideoIdByDB = () => new Promise((resolve, reject) => {
  const selectQuery = `
    SELECT * 
