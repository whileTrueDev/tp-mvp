...
// # 평균 시청자 수를 계산하기 위한 join
// SELECT streamerId, streamerName, streamId, ROUND(AVG(viewerCount)) AS viewer
// FROM TwitchStreamDetails
// JOIN TwitchStreams USING (streamId)
// WHERE endedAt > DATE_SUB(NOW(), INTERVAL 1 DAY)
// AND needCollect = 1
// GROUP BY streamId
// HAVING viewer > 10
...
const getStreamData = ({userMap, creators}) => new Promise((resolve, reject)=>{
  const conditionQuery = creators.reduce((str, element, index)=>{
    return (index == 0 ? '(' + str : str + ',' )  + `'${element.twitchId}'`;
  },'');
