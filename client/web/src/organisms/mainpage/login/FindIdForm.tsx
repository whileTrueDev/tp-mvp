...
export default function FindAccountForm(): JSX.Element {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>): void {
    ...
    if (usernameRef.current && usermailRef.current) {
      ...
      getRequest({
        params: { type: 'id', username, usermail }
      }).then((res) => {
        if (res.data) {
  ...
  // **************************************************
  // iamport 본인인증 
  const [certRes, certGetRequest] = useAxios(
    '/auth/certification', { manual: true }
  );
  const iamport = useIamportCertification(() => {
    // iamport 본인인증 이후 실행될 Id 조회 함수
    certGetRequest({
      params: { type: 'id', impUid: iamport.impUid }
    }).then((res) => {
      if (res.data) {
        const { userId } = res.data;
        const asteriskedUserId = transformIdToAsterisk(userId);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      {helperOpen && certRes.error && (
        <div style={{ marginTop: 32, minWidth: 300, maxWidth: 500, }}>
          <LoginHelper text="본인인증에 성공했으나 아이디 정보를 불러오는 도중에 오류가 발생했습니다. support@mytruepoint.com으로 문의바랍니다." />
        </div>
      )}
      ...
      <div className={classnames(classes.box, classes.content)}>
        ...
        <form className={classes.content} onSubmit={handleSubmit}>
          ...
          <Button
            ...
            style={{ color: 'white' }}
            disabled={activeStep === 1 && (loading || certRes.loading)}
            type="submit"
          >
      ...
      {activeStep === 1 && (loading || certRes.loading) && (
