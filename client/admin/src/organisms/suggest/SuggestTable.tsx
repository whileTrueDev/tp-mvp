...
interface props {
  ...
  handleRepliedData: (Data: any) =>void;
}
...
export default function SuggestTable(props: props) {
  const { suggestData, handleData, handleEditModeOff, handleRepliedData } = props;
  ...
  return (
    <MaterialTable
      title="기능 제안"
      columns={[
        { title: '카테고리', field: 'topic', render: rowData => (<Typography>{rowData.topic}</Typography>) },
        {
          title: '제목',
          field: 'title',
          render: rowData => (
